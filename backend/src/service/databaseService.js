...
async function getRecipeList() {
  try {
    const pool = await connection.getPool();
    const data = await pool.request().query(`select recipe_name, category, instructions, picture_location 
            FROM Recipes rec 
...
async function getRecipeIngredients(recipeName) {
  try {
    const pool = await connection.getPool();
    const data = await pool.request().query(`select ingredient_name, amount, measurement_type FROM Recipes rec 
